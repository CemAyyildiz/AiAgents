    ...
    return (
      <div className="card border-gray-200">
        <div className="flex items-center">
          ...
          <span className="text-gray-600">Yapılandırma kontrol ediliyor...</span>
        </div>
      </div>
    )
  }
      ...
      <div className="flex items-start">
        ...
        <div className="flex-1">
          <h3 className={`font-semibold ${
            config.hasOpenAIKey ? 'text-green-900' : 'text-yellow-900'
          }`}>
            {config.hasOpenAIKey ? 'OpenAI API Aktif' : 'OpenAI API Yapılandırılmamış'}
          </h3>
          <p className={`text-sm mt-1 ${
            config.hasOpenAIKey ? 'text-green-700' : 'text-yellow-700'
          }`}>
            {config.hasOpenAIKey ? (
              <>
                Agent'larınız gerçek AI yanıtları verebilir. 
                <span className="font-medium"> Tam işlevsellik aktif!</span>
              </>
            ) : (
              <>
                OpenAI API key'i yapılandırılmamış. 
                <span className="font-medium"> Simüle edilmiş yanıtlar kullanılıyor.</span>
              </>
            )}
          </p>
          {!config.hasOpenAIKey && (
            <div className="mt-3 p-3 bg-white border border-yellow-200 rounded-lg">
              ...
              <ol className="text-xs text-yellow-700 space-y-1 ml-6">
